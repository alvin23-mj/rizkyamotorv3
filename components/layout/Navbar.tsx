  ...
  return (
    <header className="sticky top-0 z-50 bg-white shadow-sm border-b border-slate-200">
      ...
      <div className="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8 h-[78px] flex items-center justify-between">
        {/* Brand Logo (Spanning Navbar 1 & Navbar 2) */}
        <Link href="/" className="flex items-center shrink-0 pr-4 sm:pr-6 relative z-30 -mt-11 h-[122px]">
          {/* eslint-disable-next-line @next/next/no-img-element */}
          <img
            src={settings?.logoUrl || '/logo.png'}
            alt={settings?.name || 'Rizkya Motor'}
            className={`h-20 sm:h-24 lg:h-[96px] max-w-[280px] sm:max-w-[340px] object-contain transition-opacity duration-300 mix-blend-multiply ${
      ...
      {/* Popover / Overlay Search Bar */}
      {searchModalOpen && (
        <div className="bg-slate-100 border-t border-b border-slate-200 py-3 px-4 transition-all">
          <div className="max-w-[1400px] mx-auto">
            <form onSubmit={handleSearchSubmit} className="flex items-center gap-3 relative max-w-2xl mx-auto">
              ...
              <input
                ...
                className="w-full bg-white text-slate-800 text-xs sm:text-sm font-medium pl-12 pr-10 py-3 rounded-xl border border-slate-200 shadow-sm focus:outline-none focus:ring-2 focus:ring-slate-900"
              />
              <button
                type="button"
      ...
      {mobileMenuOpen && (
        <div className="lg:hidden bg-white border-b border-slate-200 px-4 py-6 space-y-4">
          <form onSubmit={handleSearchSubmit} className="relative">
            ...
            <input
              ...
              className="w-full bg-slate-50 text-slate-800 text-xs pl-9 pr-3 py-2.5 rounded-lg border border-slate-200"
            />
          </form>

          <nav className="flex flex-col space-y-2 text-xs font-bold text-slate-800">
            <Link href="/" onClick={() => setMobileMenuOpen(false)} className="py-2 border-b border-slate-100">
              BERANDA
            </Link>
            <Link href="/cars" onClick={() => setMobileMenuOpen(false)} className="py-2 border-b border-slate-100">
              KATALOG MOBIL
            </Link>
            <Link href="/schedule" onClick={() => setMobileMenuOpen(false)} className="py-2 border-b border-slate-100">
              JADWAL TEST DRIVE
            </Link>
            <Link href="/sell" onClick={() => setMobileMenuOpen(false)} className="py-2 border-b border-slate-100">
              JUAL MOBIL
            </Link>
            <Link href="/compare" onClick={() => setMobileMenuOpen(false)} className="py-2 border-b border-slate-100">
              KOMPARASI
            </Link>
